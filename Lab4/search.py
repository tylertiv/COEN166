...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    found = problem.goalTest(startState)
    while not found:
        actions = problem.getActions(currentNode.state)
        for move in actions:
            nextState = problem.getResult(currentNode.state, move)
            if nextState not in visited:
                # find path cost and create new node in search tree
                pathCost = problem.getCost(currentNode.state, move)
                newNode = Node(nextState, currentNode, move, pathCost)

                # find heuristic value using manhattanHeuristic algorithm
                heuristicCost = heuristic(nextState, problem)
                frontier.push(newNode, pathCost+heuristicCost)

        #update which nodes have been visited and choose current node
        visited.append(currentNode.state)
        currentNode = frontier.pop()    # priority queue pops node with lowest pathcost+heuristic value
        found = problem.goalTest(currentNode.state)

    solution = []
    """ currentNode is at a solution state, so iterate backwards from
        parent to parent until the initial state is reached"""
